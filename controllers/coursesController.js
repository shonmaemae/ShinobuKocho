...
exports.createCourse = (req, res) => {
  ...
  // Ensure that the unique course code and title are provided.
  ...
  db.query(sql, [code, title, units || null], (err, result) => {
    if (err) {
      // Handle the case where the course code is a duplicate.
...
exports.updateCourse = (req, res) => {
  ...
  db.query('UPDATE courses SET code=?, title=?, units=? WHERE id=?',
    ...
    (err, result) => {
      if (err) {
        // Handle potential duplicate code on update.
        if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ error: 'code must be unique' });
        return res.status(500).json({ error: err.message });
      }
